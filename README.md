...
[`lean/Sts.lean`](lean/Sts.lean) is where an arriving message reaches its handler:
`Store.receive` matches on the kind of wire object and names the routine, and `reactions`
does the same for a tick and a wake. The guide walks it with the code —
[finding the routine that handles a message](lean/Spec/doc/guide.md#finding-the-routine-that-handles-a-message).

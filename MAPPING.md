...
11 and 21 except the store-facing `FC`, and Lemmas 1 to 11 with Theorem 5.** Every one of them is
proved: Lemma 8's five statements and Lemma 9's two included. Lemma 9's row stays 🟡 because the
E1 half of its second sentence is not stated at all, not because anything written down is
outstanding.
...
| `lem:target-bit-compression` | Lem. 9 | `lemTargetBitCompression` — the first sentence, as the two tallies' provenance (a re-export of `Witnessed` over `postState`); `lemTargetBitCompressionEvidence` — the second sentence's E2 half, proved from that re-export applied on each chain; its E1 half is absent | 2 prose | 🟡 partial |

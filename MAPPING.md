...
Definitions 11 and 21, and Lemmas 1 to 7** — Lemmas 1 to 6 proved and each covering its paper
sentence in full, Lemma 7 written down with its proof outstanding. Every other Lean cell is `—`. The row set is the paper's, not a plan: a row appearing here says
...
| `lem:height-target-freshness` | Lem. 7 | `lemHeightTargetFreshness` — the post-state at `T` as an existential over `BlockPostState`, not the paper's definite description | 1 | 🔨 stated |

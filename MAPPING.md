...
Definitions 11 and 21, and Lemmas 1 to 7**, all proved — the first six covering their paper sentence
in full, Lemma 7 narrower in one respect its row names. Every other Lean cell is `—`. The row set is the paper's, not a plan: a row appearing here says
...
| `lem:height-target-freshness` | Lem. 7 | `lemHeightTargetFreshness` — the post-state at `T` as an existential over `BlockPostState`, not the paper's definite description | 1 | 🟡 partial |

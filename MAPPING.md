...
Definitions 11 and 21, and Lemmas 1 to 5** — the first four proved and each covering its paper
sentence in full, Lemma 5 written down with its proof outstanding. Every other Lean cell is `—`. The row set is the paper's, not a plan: a row appearing here says
...
| Paper | № | Lean |
| --- | --- | --- |
| `def:certificates` (`JC` only) | Def. 21 | `JustificationCertificate`, `IncludedOn` — `BlockPostState` in place of `σ[·]`, so the certificate is weaker than the paper's |
...
| `lem:target-uniqueness` | Lem. 5 | `lemTargetUniqueness` — the paper's "unless" as the conclusion, with the evidence included on both chains | 6 | 🔨 stated |

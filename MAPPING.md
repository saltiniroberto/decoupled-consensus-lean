...
Lemmas 1 to 4**, each covering its paper sentence in full. Every other Lean cell is `—`. The row set is the paper's, not a plan: a row appearing here says
...
| `lem:finalized-before-justified` | Lem. 4 | `lemFinalizedBeforeJustified` and `lemFinalizedBeforeJustifiedActionState`, the sentence's two subjects | 3 | ✅ proved |

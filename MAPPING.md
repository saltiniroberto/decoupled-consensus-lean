...
**What is rendered so far: Figures 1 and 2, the numbered definitions those two read, and
Lemmas 1 to 4** — the first three fully, Lemma 4 over the block post-state half of its sentence.
...
| `lem:finalized-before-justified` | Lem. 4 | `lemFinalizedBeforeJustified` over a block post-state; the paper also says "and finality action state", which is a statement to widen and needs no new assumption | 3 | 🟡 partial |

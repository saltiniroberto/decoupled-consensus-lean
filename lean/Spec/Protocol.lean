...
structure RoundState (Node Root : Type) where
  ...
  /-- `X_u^1`, the vote-time snapshot. -/
  X1 : Finset (Attestation Node Root)
  /-- `X_u^2`, the grade-0 view, fixed at `d_r + 2Δ`. -/
  X2 : Finset (Attestation Node Root)
...
structure ValidatorState (Node Root : Type) where
  ...
  /-- The `X⁻` snapshot for the round about to open, taken at `d_{r+1} − Δ`. -/
  nextXm : Finset (Attestation Node Root)
  ...
  | .tick =>
      ...
      | some r =>
          ...
          if t = d then
            ...
            { state := { st with round :=
                { r := r, Xm := st.nextXm, X0 := st.atts,
                  selSnap := sel, voteSnap := sel,
                  X1 := ∅, X2 := ∅, Vm := ∅, frozen := st.round.frozen,
            ...
            let L := lowerRoot st.round.selSnap Svote wit st.round.Xm X1 r pf
            let vr := stableRoot Svote wit L X1 r pf prop
            ...
            let st' := { st with
              ...
              round := { st.round with
                X1 := X1, voteSnap := Svote, root := vr.root,
...
def protocol : Protocol Node (StoreMsg Node Root) (ValidatorState Node Root) Empty where
  init _ :=
    { store := Store.gen, hist := .gen, snap := Store.gen,
      atts := ∅, gvotes := ∅, props := ∅, prevProposal := none, nextXm := ∅,
      round := { r := 0, Xm := ∅, X0 := ∅, X1 := ∅, X2 := ∅,

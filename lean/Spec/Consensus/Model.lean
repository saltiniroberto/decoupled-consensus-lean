...
repository root. It **supersedes `consensus.pdf`**, which this subtree rendered until
2026-08-22; that draft's six figure files are gone from the tree and remain in git history.
The two drafts share a title and little else: the newer one is block-only Goldfish with one
store built up in three layers, where the older had a candidate tree, grades and round roots.
...
which is frozen as the record of `latex-specs`. Decided 2026-08-19 and unchanged.
...
rendered as a `Nat` carried by the block. That replaces the old draft's `BlockHash` class,
which supplied the same tiebreak abstractly.

...
* **`Finset.filterM` could not be used over it otherwise.** That fold needs its combining
  operation commutative, and the failure-failure case needs the two failures to be *equal* —
  which is `Subsingleton Error`. Give `Error` a payload and `Std.Commutative` becomes false,
  not merely unproved, because the result would then depend on which failure the traversal
  met first.
...
/-- The one failure of this rendering, thrown by every routine that can fail and carrying
    ...
    Timing is deliberately not a failure. A routine's instant is an input precondition, a

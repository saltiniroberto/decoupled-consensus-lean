...
It is a duplicate of `Spec/Defs/Notation.lean` under this subtree's own namespace, because
the new rendering shares nothing with the old one — not even by import — so the frozen
files stay untouched (decided 2026-08-19). The macros are protocol-free, so duplication
carries no claim that could drift. What that file decided, and this copy keeps:

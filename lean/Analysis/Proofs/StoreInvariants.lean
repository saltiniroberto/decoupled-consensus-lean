...
theorem F_preceq_FGWalkStart {S : Store Node Root} (h : S.F ⪯ S.J) : S.F ⪯ S.walkStartFromFGVotes := by
  unfold Store.walkStartFromFGVotes
  split_ifs with hc
  · exact h
  · exact Preceq.refl _
...
theorem mem_candidates {S : Store Node Root} {B : Blk Node Root} :
    (B ∈ (viableTree S).filter fun B =>
        S.walkStartFromFGVotes ⪯ B ∧ (S.σ B).any fun st => st.h ≥ S.hmax - 1) ↔
      B ∈ viableTree S ∧ S.walkStartFromFGVotes ⪯ B ∧ (get st from S.σ B; st.h ≥ S.hmax - 1) := by
...
theorem getConfirmed_spec [Omega Node Root] (S : Store Node Root) :
    getConfirmed S = S.walkStartFromFGVotes ∨
      (getConfirmed S ∈ viableTree S ∧ S.walkStartFromFGVotes ⪯ getConfirmed S ∧
        (get st from S.σ (getConfirmed S); st.h ≥ S.hmax - 1)) := by
  have h := (Omega.choose (insert S.walkStartFromFGVotes ((viableTree S).filter fun B =>
      S.walkStartFromFGVotes ⪯ B ∧ (S.σ B).any fun st => st.h ≥ S.hmax - 1))
...
theorem getConfirmed_congr [Omega Node Root] {S S' : Store Node Root} (hR : S.walkStartFromFGVotes = S'.walkStartFromFGVotes)
    (h : ∀ C, (C ∈ viableTree S ∧ S.walkStartFromFGVotes ⪯ C ∧ (get st from S.σ C; st.h ≥ S.hmax - 1)) ↔
      (C ∈ viableTree S' ∧ S'.walkStartFromFGVotes ⪯ C ∧ (get st from S'.σ C; st.h ≥ S'.hmax - 1))) :

...
`F ⪯ Σ.F ⪯ Σ.walkStart ⪯ C`, which is the contradiction again.
-/
...
theorem lockIn_store [PositiveWeight Node] {S : Store Node Root} (hinv : StoreInv S)
    ...
    (F ⪯ S.J ∧ F ∈ viableTree S ∧
      ∀ C, (C ∈ viableTree S ∧ S.walkStart ⪯ C ∧ (get st from S.σ C; st.h ≥ S.hmax - 1)) → F ⪯ C) ∨
  ...
  case inr => exact Or.inr hev
  by_cases h3 : ∀ C, (C ∈ viableTree S ∧ S.walkStart ⪯ C ∧ (get st from S.σ C; st.h ≥ S.hmax - 1)) → F ⪯ C
  ...
  have hR : S.hmax ≠ S.h_j + 1 := by
    intro he
    have hRJ : S.walkStart = S.J := by unfold Store.walkStart; rw [if_pos he]
    exact hFC (Preceq.trans h1 (by rw [← hRJ]; exact hC.2.1))
  have hRF : S.walkStart = S.F := by unfold Store.walkStart; rw [if_neg hR]

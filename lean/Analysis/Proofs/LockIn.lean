...
`F ⪯ Σ.F ⪯ Σ.walkStartFromFGVotes ⪯ C`, which is the contradiction again.
-/
...
theorem lockIn_store [PositiveWeight Node] {S : Store Node Root} (hinv : StoreInv S)
    ...
    (F ⪯ S.J ∧ F ∈ viableTree S ∧
      ∀ C, (C ∈ viableTree S ∧ S.walkStartFromFGVotes ⪯ C ∧ (get st from S.σ C; st.h ≥ S.hmax - 1)) → F ⪯ C) ∨
  ...
  case inr => exact Or.inr hev
  by_cases h3 : ∀ C, (C ∈ viableTree S ∧ S.walkStartFromFGVotes ⪯ C ∧ (get st from S.σ C; st.h ≥ S.hmax - 1)) → F ⪯ C
  ...
  have hR : S.hmax ≠ S.h_j + 1 := by
    intro he
    have hRJ : S.walkStartFromFGVotes = S.J := by unfold Store.walkStartFromFGVotes; rw [if_pos he]
    exact hFC (Preceq.trans h1 (by rw [← hRJ]; exact hC.2.1))
  have hRF : S.walkStartFromFGVotes = S.F := by unfold Store.walkStartFromFGVotes; rw [if_neg hR]

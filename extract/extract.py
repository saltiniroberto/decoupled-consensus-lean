...
% one pseudocode line: number, indent, content, optional right note
\newcommand{\figline}[3]{\par\noindent\hangindent=8em\hangafter=1%
\makebox[2.2em][r]{\scriptsize\color{black!55}#1}\hspace{0.8em}\hspace*{#2}#3\par}
\newenvironment{figblock}{\par\medskip\small
\setlength{\parskip}{1pt}\setlength{\parindent}{0pt}}{\par\medskip}
...
def render_figure(tex, fig_no, fig_title, routines_lines, prose_rw):
    ...
    for lines in routines_lines:
        for label, indent, spans, note in lines:
            pad = f"{(indent + 1) * 1.3:.1f}em" if indent >= 0 else "0em"
            content = latex_spans(spans)
